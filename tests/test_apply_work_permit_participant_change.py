...
class ApplyWorkPermitParticipantChangeTests(unittest.TestCase):
    ...
    def test_apply_work_permit_participant_change_reissues_permit_when_more_than_half_is_replaced(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_work_permit_record(context.database_path, 'ND-APPLY-001', 'Вогневі роботи', 'Дільниця А', '2099-04-10 08:00', '2099-04-20 18:00', 'Майстер', '', '', '', 'Початковий склад бригади', participants=(WorkPermitParticipant('0001', 'Коваль Олена', WorkPermitParticipantRole.EXECUTOR), WorkPermitParticipant('0002', 'Іваненко Сергій', WorkPermitParticipantRole.TEAM_MEMBER), WorkPermitParticipant('0003', 'Петренко Андрій', WorkPermitParticipantRole.TEAM_MEMBER)), access_role=AccessRole.INSPECTOR)
            source_record = next((record for record in load_work_permit_registry(context.database_path) if record.permit_number == 'ND-APPLY-001'))
            outcome = apply_work_permit_participant_change(context.database_path, int(source_record.record_id), (WorkPermitParticipant('0001', 'Коваль Олена', WorkPermitParticipantRole.EXECUTOR), WorkPermitParticipant('0004', 'Шевченко Михайло', WorkPermitParticipantRole.TEAM_MEMBER), WorkPermitParticipant('0005', 'Бондар Тетяна', WorkPermitParticipantRole.OBSERVER)), access_role=AccessRole.INSPECTOR)
    ...
    def test_apply_work_permit_participant_change_rejects_auto_reissue_for_expired_permit(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            application_paths = build_application_paths(Path(temporary_directory))
            context = initialize_application(application_paths)
            create_work_permit_record(context.database_path, 'ND-APPLY-002', 'Вогневі роботи', 'Дільниця Б', '2000-04-10 08:00', '2000-04-20 18:00', 'Майстер', '', '', '', 'Прострочений наряд', participants=(WorkPermitParticipant('0001', 'Коваль Олена', WorkPermitParticipantRole.EXECUTOR), WorkPermitParticipant('0002', 'Іваненко Сергій', WorkPermitParticipantRole.TEAM_MEMBER), WorkPermitParticipant('0003', 'Петренко Андрій', WorkPermitParticipantRole.TEAM_MEMBER)), access_role=AccessRole.INSPECTOR)
            source_record = next((record for record in load_work_permit_registry(context.database_path) if record.permit_number == 'ND-APPLY-002'))
            with self.assertRaisesRegex(ValueError, 'Строк дії наряду вже сплив'):
                apply_work_permit_participant_change(context.database_path, int(source_record.record_id), (WorkPermitParticipant('0001', 'Коваль Олена', WorkPermitParticipantRole.EXECUTOR), WorkPermitParticipant('0004', 'Шевченко Михайло', WorkPermitParticipantRole.TEAM_MEMBER), WorkPermitParticipant('0005', 'Бондар Тетяна', WorkPermitParticipantRole.OBSERVER)), access_role=AccessRole.INSPECTOR)
            shut_down_logging()
