...
class RecordWorkPermitDailyCheckTests(unittest.TestCase):
    ...
    def test_record_work_permit_daily_check_adds_check_and_writes_audit_log(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_work_permit_record(context.database_path, 'ND-CHK-001', 'Вогневі роботи', 'Дільниця А', '2099-04-10 08:00', '2099-04-12 18:00', 'Майстер', 'Інспектор', '0001', 'executor', 'Багатоденний наряд', access_role=AccessRole.INSPECTOR)
            ...
            record_work_permit_daily_check(context.database_path, int(created_record.record_id), '11.04.2099 09:30', 'Старший майстер', 'Місце робіт перевірено', access_role=AccessRole.INSPECTOR)
            ...
            audit_events = connection.execute("SELECT event_type FROM audit_log WHERE event_type = 'work_permit.daily_check_recorded';").fetchall()
            connection.close()
    ...
    def test_record_work_permit_daily_check_rejects_second_check_for_same_day(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            application_paths = build_application_paths(Path(temporary_directory))
            context = initialize_application(application_paths)
            create_work_permit_record(context.database_path, 'ND-CHK-002', 'Вогневі роботи', 'Дільниця Б', '2099-04-10 08:00', '2099-04-12 18:00', 'Майстер', 'Інспектор', '0001', 'executor', 'Багатоденний наряд', access_role=AccessRole.INSPECTOR)
            ...
            record_work_permit_daily_check(context.database_path, int(created_record.record_id), '2099-04-11 09:00', 'Старший майстер', access_role=AccessRole.INSPECTOR)
            with self.assertRaisesRegex(ValueError, 'вже зафіксовано'):
                record_work_permit_daily_check(context.database_path, int(created_record.record_id), '2099-04-11 15:00', 'Старший майстер', access_role=AccessRole.INSPECTOR)
            shut_down_logging()
