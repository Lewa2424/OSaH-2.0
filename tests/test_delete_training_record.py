...
class DeleteTrainingRecordTests(unittest.TestCase):
    ...
    def test_delete_training_record_removes_record_and_writes_audit_log(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_training_record(database_path=context.database_path, employee_personnel_number='0001', training_type='introductory', event_date_text='2026-04-10', next_control_date_text='2026-06-10', conducted_by='Інспектор з ОП', note_text='Початковий запис', access_role=AccessRole.INSPECTOR)
            created_record = next((training_record for training_record in load_training_registry(context.database_path) if training_record.employee_personnel_number == '0001' and training_record.note_text == 'Початковий запис'))
            training_total_before_delete = len(load_training_registry(context.database_path))
            delete_training_record(database_path=context.database_path, record_id=int(created_record.record_id), access_role=AccessRole.INSPECTOR)
            remaining_records = load_training_registry(context.database_path)
            connection = sqlite3.connect(context.database_path)
            audit_events = connection.execute("SELECT event_type FROM audit_log WHERE event_type = 'training.deleted';").fetchall()
            connection.close()
