...
class SaveMailSettingsTests(unittest.TestCase):
    ...
    def test_save_mail_settings_persists_and_loads_values(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            save_mail_settings(context.database_path, MailSettings(daily_report_enabled=True, smtp_host='smtp.example.com', smtp_port=587, smtp_username='user@example.com', smtp_password='secret', sender_email='sender@example.com', recipient_email='boss@example.com', use_tls=True, last_sent_date='2026-04-10'), access_role=AccessRole.INSPECTOR)
            mail_settings = load_mail_settings(context.database_path)
            self.assertFalse(mail_settings.daily_report_enabled)
    ...
    def test_save_mail_settings_does_not_write_secret_values_to_audit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application_paths = build_application_paths(Path(temporary_directory))
            context = initialize_application(application_paths)
            save_mail_settings(context.database_path, MailSettings(daily_report_enabled=True, smtp_host='smtp.example.com', smtp_port=587, smtp_username='user@example.com', smtp_password='super-secret-password', sender_email='sender@example.com', recipient_email='boss@example.com', use_tls=True, last_sent_date='2026-04-10'), access_role=AccessRole.INSPECTOR)
            audit_log_entries = load_audit_log_entries(context.database_path, limit=5)
            mail_audit_entry = next((audit_log_entry for audit_log_entry in audit_log_entries if audit_log_entry.event_type == 'mail.settings_updated'))
    ...
    def test_save_mail_settings_derives_smtp_values_from_sender_email(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application_paths = build_application_paths(Path(temporary_directory))
            context = initialize_application(application_paths)
            save_mail_settings(context.database_path, MailSettings(daily_report_enabled=True, smtp_host='', smtp_port=0, smtp_username='', smtp_password='secret', sender_email='sender@example.com', recipient_email='boss@example.com', use_tls=False, last_sent_date='2026-04-10'), access_role=AccessRole.INSPECTOR)
            mail_settings = load_mail_settings(context.database_path)
            self.assertEqual(mail_settings.smtp_host, 'smtp.example.com')
            self.assertEqual(mail_settings.smtp_port, 587)
