...
class WorkPermitTargetTrainingTests(unittest.TestCase):
    ...
    def test_done_passed_creates_targeted_training_record_and_audit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_work_permit_record(context.database_path, permit_number, 'Вогневі роботи', 'Дільниця А', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Плановий наряд', target_training_status='done_passed', target_training_date_text='09.05.2026', target_training_conducted_by='Коваль О.В.', target_training_note='Інструктаж проведено перед початком робіт.', access_role=AccessRole.INSPECTOR)
                ...
                training_record = next((record for record in load_training_registry(context.database_path) if record.source_key == f'work_permit_target_training:{permit_record.record_id}:0001'))
    ...
    def test_done_failed_creates_not_allowed_training_and_notifications(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_work_permit_record(context.database_path, permit_number, 'Газонебезпечні роботи', 'Дільниця Б', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Роботи під контролем', target_training_status='done_failed', target_training_date_text='09.05.2026', target_training_conducted_by='Коваль О.В.', access_role=AccessRole.INSPECTOR)
                permit_record = next((record for record in load_work_permit_registry(context.database_path) if record.permit_number == permit_number))
                training_record = next((record for record in load_training_registry(context.database_path) if record.source_key == f'work_permit_target_training:{permit_record.record_id}:0001'))
    ...
    def test_repeated_save_updates_without_duplicates(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_work_permit_record(context.database_path, permit_number, 'Висотні роботи', 'Дільниця В', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Перше збереження', target_training_status='done_passed', target_training_date_text='09.05.2026', target_training_conducted_by='Коваль О.В.', access_role=AccessRole.INSPECTOR)
                permit_record = next((record for record in load_work_permit_registry(context.database_path) if record.permit_number == permit_number))
                update_work_permit_record(context.database_path, int(permit_record.record_id), permit_record.permit_number, permit_record.work_kind, permit_record.work_location, '10.05.2026 08:00', '10.05.2026 12:00', permit_record.responsible_person, permit_record.issuer_person, '0001', 'executor', 'Друге збереження', target_training_status='done_passed', target_training_date_text='09.05.2026', target_training_conducted_by='Петренко І.В.', target_training_note='Оновлено під час повторного збереження.', access_role=AccessRole.INSPECTOR)
                connection = sqlite3.connect(context.database_path)
                count_row = connection.execute(f"\n                    SELECT COUNT(*)\n                    FROM trainings\n                    WHERE source_key = 'work_permit_target_training:{permit_record.record_id}:0001'\n                    ").fetchone()
                updated_row = connection.execute(f"\n                    SELECT conducted_by, basis_note\n                    FROM trainings\n                    WHERE source_key = 'work_permit_target_training:{permit_record.record_id}:0001'\n                    LIMIT 1\n                    ").fetchone()
                connection.close()
    ...
    def test_multi_participant_permit_creates_targeted_training_for_each_participant(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            try:
                create_work_permit_record(context.database_path, 'AUTO-WP-MULTI-001', 'Висотні роботи', 'Дільниця Ж', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Бригадний наряд', target_training_status='done_passed', target_training_date_text='09.05.2026', target_training_conducted_by='Коваль О.В.', participants=(WorkPermitParticipant('0001', 'Коваль Олена Вікторівна', WorkPermitParticipantRole.EXECUTOR), WorkPermitParticipant('0002', 'Іваненко Ігор Олександрович', WorkPermitParticipantRole.TEAM_MEMBER)), access_role=AccessRole.INSPECTOR)
                ...
                created_records = [record for record in load_training_registry(context.database_path) if record.source_module == 'work_permits' and record.source_record_id == permit_record.record_id]
                self.assertEqual(len(created_records), 2)
                self.assertEqual({record.employee_personnel_number for record in created_records}, {'0001', '0002'})
            finally:
                shut_down_logging()
    ...
    def test_not_done_does_not_create_training_record(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_work_permit_record(context.database_path, permit_number, 'Електророботи', 'Дільниця Г', '10.05.2099 08:00', '10.05.2099 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Інструктаж ще не проведено', target_training_status='not_done', access_role=AccessRole.INSPECTOR)
    ...
    def test_downgrade_to_not_done_archives_linked_training_records(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_work_permit_record(context.database_path, permit_number, 'Висотні роботи', 'Дільниця И', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Початково інструктаж проведено', target_training_status='done_passed', target_training_date_text='09.05.2026', target_training_conducted_by='Коваль О.В.', access_role=AccessRole.INSPECTOR)
                permit_record = next((record for record in load_work_permit_registry(context.database_path) if record.permit_number == permit_number))
                created_records = [record for record in load_training_registry(context.database_path) if record.source_module == 'work_permits' and record.source_record_id == permit_record.record_id]
                self.assertEqual(len(created_records), 1)
                update_work_permit_record(context.database_path, int(permit_record.record_id), permit_record.permit_number, permit_record.work_kind, permit_record.work_location, '10.05.2026 08:00', '10.05.2026 12:00', permit_record.responsible_person, permit_record.issuer_person, '0001', 'executor', 'Інструктаж скасовано', target_training_status='not_done', target_training_date_text='', target_training_conducted_by='', target_training_note='', access_role=AccessRole.INSPECTOR)
                remaining_records = [record for record in load_training_registry(context.database_path) if record.source_module == 'work_permits' and record.source_record_id == permit_record.record_id]
                self.assertEqual(len(remaining_records), 0)
                archived_records = [record for record in load_training_registry(context.database_path, include_archived=True) if record.source_module == 'work_permits' and record.source_record_id == permit_record.record_id]
    ...
    def test_legacy_status_does_not_create_noise(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_work_permit_record(context.database_path, permit_number, 'Ремонтні роботи', 'Дільниця Д', '10.05.2099 08:00', '10.05.2099 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Старий сценарій без фіксації інструктажу', target_training_status='legacy_not_tracked', access_role=AccessRole.INSPECTOR)
                trainings = load_training_registry(context.database_path)
                self.assertFalse(any((record.source_module == 'work_permits' for record in trainings)))
                connection = sqlite3.connect(context.database_path)
    ...
    def test_done_status_requires_date_and_conductor(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            try:
                with self.assertRaisesRegex(ValueError, 'Для проведеного цільового інструктажу потрібно вказати дату та особу, яка його провела.'):
                    create_work_permit_record(context.database_path, 'AUTO-WP-VALIDATE-001', 'Вогневі роботи', 'Дільниця Е', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Без дати', target_training_status='done_passed', target_training_conducted_by='Коваль О.В.', access_role=AccessRole.INSPECTOR)
                with self.assertRaisesRegex(ValueError, 'Для проведеного цільового інструктажу потрібно вказати дату та особу, яка його провела.'):
                    create_work_permit_record(context.database_path, 'AUTO-WP-VALIDATE-002', 'Вогневі роботи', 'Дільниця Е', '10.05.2026 08:00', '10.05.2026 12:00', 'Майстер', 'Інженер з ОП', '0001', 'executor', 'Без проводившого', target_training_status='done_failed', target_training_date_text='09.05.2026', access_role=AccessRole.INSPECTOR)
            finally:
                shut_down_logging()
