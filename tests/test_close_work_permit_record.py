...
class CloseWorkPermitRecordTests(unittest.TestCase):
    ...
    def test_close_work_permit_record_marks_record_as_closed_and_writes_audit_log(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_work_permit_record(database_path=context.database_path, permit_number='НД-101', work_kind='Висотні роботи', work_location='Дільниця Б', starts_at_text='2099-04-10 08:00', ends_at_text='2099-04-10 10:00', responsible_person='Майстер зміни', issuer_person='Інспектор з ОП', employee_personnel_number='0001', participant_role='executor', note_text='Потрібне ручне закриття', access_role=AccessRole.INSPECTOR)
            created_record = next((work_permit_record for work_permit_record in load_work_permit_registry(context.database_path) if work_permit_record.permit_number == 'НД-101'))
            close_work_permit_record(context.database_path, int(created_record.record_id), access_role=AccessRole.INSPECTOR)
            updated_record = next((work_permit_record for work_permit_record in load_work_permit_registry(context.database_path) if int(work_permit_record.record_id) == int(created_record.record_id)))
            connection = sqlite3.connect(context.database_path)
            audit_events = connection.execute("SELECT event_type FROM audit_log WHERE event_type = 'work_permit.closed';").fetchall()
            connection.close()
