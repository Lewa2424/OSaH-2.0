        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                ...
                create_ppe_record(
                    ...
                    compliance_check_state="checked",
                )
                create_medical_record(
                    ...
                    "fit",
                    "",
                    medical_exam_basis="internal_list",
                )
...
def _clear_employee_related_records(database_path: Path, personnel_number: str) -> None:
    ...
    try:
        work_permit_ids = tuple(
            row[0]
            for row in connection.execute(
                "SELECT work_permit_id FROM work_permit_participants WHERE employee_personnel_number = ?;",
                (personnel_number,),
            ).fetchall()
        )
        connection.execute(
            "DELETE FROM work_permit_participants WHERE employee_personnel_number = ?;",
            (personnel_number,),
        )
        if work_permit_ids:
            placeholders = ", ".join("?" for _ in work_permit_ids)
            connection.execute(
                f"DELETE FROM work_permits WHERE id IN ({placeholders}) AND id NOT IN (SELECT work_permit_id FROM work_permit_participants);",
                work_permit_ids,
            )
