...
class CreateWorkPermitRecordTests(unittest.TestCase):
    ...
    def test_create_work_permit_record_persists_record_and_participant(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_work_permit_record(database_path=context.database_path, permit_number='НД-100', work_kind='Вогневі роботи', work_location='Дільниця А', starts_at_text='2099-04-10 08:00', ends_at_text='2099-04-10 10:00', responsible_person='Майстер зміни', issuer_person='Інспектор з ОП', employee_personnel_number='0001', participant_role='executor', note_text='Початковий наряд', access_role=AccessRole.INSPECTOR)
    ...
    def test_create_work_permit_record_allows_lightweight_registry_entry(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            application_paths = build_application_paths(Path(temporary_directory))
            context = initialize_application(application_paths)
            create_work_permit_record(database_path=context.database_path, permit_number='НД-101', work_kind='Ремонтні роботи', work_location='Цех 4', starts_at_text='2099-04-10 08:00', ends_at_text='2099-04-10 12:00', responsible_person='Старший майстер', issuer_person='', employee_personnel_number='', participant_role='', note_text='Лише контрольний запис', access_role=AccessRole.INSPECTOR)
            created_record = next((record for record in load_work_permit_registry(context.database_path) if record.permit_number == 'НД-101'))
    ...
    def test_create_work_permit_record_rejects_term_longer_than_fifteen_days(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaisesRegex(ValueError, '15'):
                create_work_permit_record(database_path=context.database_path, permit_number='НД-102', work_kind='Вогневі роботи', work_location='Дільниця А', starts_at_text='2099-04-10 08:00', ends_at_text='2099-04-26 10:00', responsible_person='Майстер зміни', issuer_person='Інспектор з ОП', employee_personnel_number='0001', participant_role='executor', note_text='Надто довгий наряд', access_role=AccessRole.INSPECTOR)
            shut_down_logging()
