    ...
    def test_medical_exam_basis_persists_and_loads(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_medical_record(
                ...
                "fit",
                "",
                medical_exam_basis="under_21",
                basis_text="Річний огляд",
                basis_note="Контрольний приклад",
            )

            record = next(
                record
                for record in load_medical_registry(context.database_path)
                if record.basis_note == "Контрольний приклад"
            )
    ...
    def test_not_fit_is_critical_and_restricted_is_warning(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_medical_record(
                ...
                "not_fit",
                "Заборона",
                medical_exam_basis="harmful_or_dangerous_factors",
            )
            create_medical_record(
                context.database_path,
                "0002",
