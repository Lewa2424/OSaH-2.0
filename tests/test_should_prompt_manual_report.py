...
class ShouldPromptManualReportTests(unittest.TestCase):
    ...
    def test_should_prompt_manual_report_when_time_due(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            save_manual_report_settings(database_path, _settings(enabled=True, reminder_time='08:00'), access_role=AccessRole.INSPECTOR)
    ...
    def test_should_not_prompt_if_report_already_generated_today(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            database_path = _initialize_database(Path(temporary_directory))
            save_manual_report_settings(database_path, _settings(enabled=True, reminder_time='08:00', last_generated_date='2026-05-09'), access_role=AccessRole.INSPECTOR)
    ...
    def test_should_not_prompt_if_report_skipped_today(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            database_path = _initialize_database(Path(temporary_directory))
            save_manual_report_settings(database_path, _settings(enabled=True, reminder_time='08:00', last_skipped_date='2026-05-09'), access_role=AccessRole.INSPECTOR)
            self.assertFalse(should_prompt_manual_report(database_path, datetime(2026, 5, 9, 10, 0)))
            shut_down_logging()
...
def _settings(*, enabled: bool, reminder_time: str, last_generated_date: str='', last_skipped_date: str='') -> ManualReportSettings:
    """Повертає базові налаштування ручного щоденного звіту для тестів.
    Returns baseline manual daily report settings for tests.
    """
    return ManualReportSettings(manual_reminder_enabled=enabled, manual_reminder_time=reminder_time, last_generated_date=last_generated_date, last_skipped_date=last_skipped_date, next_prompt_at='', default_save_directory='', ask_save_path_each_time=True)
