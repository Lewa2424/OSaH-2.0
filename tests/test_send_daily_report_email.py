...
class SendDailyReportEmailTests(unittest.TestCase):
    ...
    def test_send_daily_report_email_updates_last_sent_date_on_success(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            save_mail_settings(context.database_path, MailSettings(daily_report_enabled=True, smtp_host='smtp.example.com', smtp_port=587, smtp_username='user@example.com', smtp_password='secret', sender_email='sender@example.com', recipient_email='boss@example.com', use_tls=True, last_sent_date=''), access_role=AccessRole.INSPECTOR)
            ...
            with patch('osah.application.services.send_daily_report_email.smtplib.SMTP', return_value=smtp_context):
                report_copy_path, failed_email_copy_path = send_daily_report_email(context.database_path, access_role=AccessRole.INSPECTOR)
    ...
    def test_send_daily_report_email_saves_eml_after_failed_retries(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            context = initialize_application(application_paths)
            save_mail_settings(context.database_path, MailSettings(daily_report_enabled=True, smtp_host='smtp.example.com', smtp_port=587, smtp_username='user@example.com', smtp_password='secret', sender_email='sender@example.com', recipient_email='boss@example.com', use_tls=True, last_sent_date=''), access_role=AccessRole.INSPECTOR)
            with patch('osah.application.services.send_daily_report_email.smtplib.SMTP', side_effect=RuntimeError('smtp unavailable')) as smtp_mock:
                report_copy_path, failed_email_copy_path = send_daily_report_email(context.database_path, access_role=AccessRole.INSPECTOR)
            self.assertTrue(report_copy_path.exists())
