...
class RestoreBackupSnapshotTests(unittest.TestCase):
    ...
    def test_restore_backup_snapshot_restores_previous_state_and_creates_safety_copy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            source_path.write_text(json.dumps([{'personnel_number': '9003', 'full_name': 'Петренко Марія Ігорівна', 'position_name': 'Майстер дільниці', 'department_name': 'Виробнича дільниця N2', 'employment_status': 'active'}], ensure_ascii=False), encoding='utf-8')
            batch_id = create_employee_import_batch_from_file(context.database_path, source_path, access_role=AccessRole.INSPECTOR)
            apply_employee_import_batch(context.database_path, batch_id, access_role=AccessRole.INSPECTOR)
            employees_before_restore = load_employee_registry(context.database_path)
            safety_backup_path = restore_backup_snapshot(context.database_path, backup_file_path, access_role=AccessRole.INSPECTOR)
