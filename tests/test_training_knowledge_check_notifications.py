        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            try:
                create_training_record(
                    ...
                    "",
                    "Інженер з ОП",
                    "Перевірка після вступу",
                    knowledge_check_result="satisfactory",
                    work_admission_status="allowed",
                    knowledge_check_note="Допущено до роботи",
                    basis_text="Наказ №12",
                    basis_note="Тестовий запис",
                )

                created_record = next(
                    record
                    for record in load_training_registry(context.database_path)
                    if record.note_text == "Перевірка після вступу"
                )
