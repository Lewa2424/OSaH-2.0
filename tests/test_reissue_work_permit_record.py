...
class ReissueWorkPermitRecordTests(unittest.TestCase):
    ...
    def test_reissue_work_permit_record_creates_new_record_and_marks_source_as_reissued(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            reissued_record = WorkPermitRecord(
                record_id=None,
                permit_number="ND-RS-002",
                work_kind="Газонебезпечні роботи",
                work_location="Дільниця Б",
                starts_at=source_record.starts_at,
                ends_at=source_record.ends_at,
                responsible_person=source_record.responsible_person,
                issuer_person=source_record.issuer_person,
                note_text="Перевипущений наряд",
                closed_at=None,
                participants=source_record.participants,
                status=WorkPermitStatus.ACTIVE,
                target_training_status=source_record.target_training_status,
                target_training_date=source_record.target_training_date,
                target_training_conducted_by=source_record.target_training_conducted_by,
                target_training_note=source_record.target_training_note,
                basis_text=source_record.basis_text,
                basis_note=source_record.basis_note,
                base_ends_at=source_record.base_ends_at,
            )
            reissue_work_permit_record(
                context.database_path,
                int(source_record.record_id),
                reissued_record,
                "Змінено умови виконання робіт",
            )
    ...
    def test_reissue_work_permit_record_rejects_reissue_without_changed_conditions(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            same_record = WorkPermitRecord(
                record_id=None,
                permit_number="ND-RS-004",
                work_kind=source_record.work_kind,
                work_location=source_record.work_location,
                starts_at=source_record.starts_at,
                ends_at=source_record.ends_at,
                responsible_person=source_record.responsible_person,
                issuer_person=source_record.issuer_person,
    ...
    def test_reissue_work_permit_record_allows_participant_only_change(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            reissued_record = WorkPermitRecord(
                ...
                responsible_person=source_record.responsible_person,
                issuer_person=source_record.issuer_person,
                note_text="Перевипуск через зміну складу",
                closed_at=None,
                participants=(
                    WorkPermitParticipant("0001", "Коваль Олена", WorkPermitParticipantRole.EXECUTOR),
                    WorkPermitParticipant("0004", "Шевченко Михайло", WorkPermitParticipantRole.TEAM_MEMBER),
                    WorkPermitParticipant("0005", "Бондар Тетяна", WorkPermitParticipantRole.TEAM_MEMBER),
                ),
                status=WorkPermitStatus.ACTIVE,
                target_training_status=source_record.target_training_status,
                target_training_date=source_record.target_training_date,
            ...
            reissue_work_permit_record(
                ...
                int(source_record.record_id),
                reissued_record,
                "Змінено більше 50% складу бригади",
            )
