...
class ContractorWorkspaceTests(unittest.TestCase):
    ...
    def test_save_and_load_contractor_record_keeps_workers_and_scope(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            saved = save_contractor_record(context.database_path, ContractorRecord(contractor_id='', company_name='Нова компанія', contact_person='Петренко', contact_phone='+380500000001', contact_email='office@test.local', activity_status='active', note_text='Контрольний запис', enterprise_supervisor='Іваненко С.П.', work_scope_text='Цех 1, монтаж трубопроводу', workers=(ContractorWorker('w1', 'Перший Працівник', 'Монтажник', True, True, True, True), ContractorWorker('w2', 'Другий Працівник', 'Стропальник', True, False, True, True, 'Потрібен комплект ЗІЗ'))), access_role=AccessRole.INSPECTOR)
            workspace = load_contractor_workspace(context.database_path)
