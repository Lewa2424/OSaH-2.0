...
class Step10LoggingAndStabilityTests(unittest.TestCase):
    ...
    def test_audit_log_redacts_sensitive_values(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            insert_audit_log(connection, event_type='security.synthetic_sensitive_event', module_name='security', event_level='warning', actor_name='test', entity_name='test', result_status='success', description_text='password=plain-secret;hash=hash-secret;salt=salt-secret;token=token-secret')
            connection.commit()
            connection.close()
    ...
    def test_service_code_reset_is_audited_without_plain_code(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            service_code = build_service_reset_code(
                reset_request.installation_id,
                reset_request.request_counter,
    ...
    def test_failed_mail_delivery_writes_attempts_and_fallback_without_secret(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            save_mail_settings(context.database_path, MailSettings(daily_report_enabled=True, smtp_host='smtp.example.com', smtp_port=587, smtp_username='user@example.com', smtp_password='smtp-super-secret', sender_email='sender@example.com', recipient_email='boss@example.com', use_tls=True, last_sent_date=''), access_role=AccessRole.INSPECTOR)
            with patch('osah.application.services.send_daily_report_email.smtplib.SMTP', side_effect=RuntimeError('smtp down')):
                _, fallback_email_path = send_daily_report_email(context.database_path, access_role=AccessRole.INSPECTOR)
    ...
    def test_news_refresh_does_not_create_control_notifications(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_news_source(context.database_path, 'Trusted NPA', 'https://example.com/npa.xml', NewsSourceKind.NPA, access_role=AccessRole.INSPECTOR)
            refresh_news_sources(context.database_path, lambda _: (RssFeedEntry(title_text="Нове роз'яснення з питань охорони праці", link_url='https://example.com/npa-1', published_at_text='2026-04-10T10:00:00'),), access_role=AccessRole.INSPECTOR)
    ...
    def test_restore_missing_backup_fails_without_safety_copy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaises(ValueError):
                restore_backup_snapshot(context.database_path, missing_backup_path, access_role=AccessRole.INSPECTOR)
    ...
    def test_cancel_work_permit_without_reason_does_not_write_success_audit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            create_work_permit_record(context.database_path, 'ND-NEG-1', 'Вогневі роботи', 'Дільниця А', '2099-04-10 08:00', '2099-04-10 12:00', 'Майстер', 'Інспектор', '0001', 'executor', '', access_role=AccessRole.INSPECTOR)
            with self.assertRaises(ValueError):
                cancel_work_permit_record(context.database_path, 1, '', access_role=AccessRole.INSPECTOR)
            audit_entries = load_audit_log_entries(context.database_path, 50)
