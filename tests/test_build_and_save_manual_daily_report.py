...
class BuildAndSaveManualDailyReportTests(unittest.TestCase):
    ...
    def test_build_and_save_manual_daily_report_creates_user_file_and_internal_copy(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            save_result = build_and_save_manual_daily_report(context.database_path, target_path, access_role=AccessRole.INSPECTOR)
    ...
    def test_manual_report_file_creation_writes_audit(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self.assertTrue(any((entry.event_type == 'report.file_created' and str(save_result.user_file_path) in entry.description_text for entry in audit_entries)))
            shut_down_logging()
