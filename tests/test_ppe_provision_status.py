...
class PpeProvisionStatusTests(unittest.TestCase):
    ...
    def test_required_not_issued_is_problem_status(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            create_ppe_record(context.database_path, '0001', 'Щиток', True, False, '2026-05-01', '2026-12-01', '1', 'Не видано', provision_status='required_not_issued', basis_text='Норма видачі', basis_note='Критична нестача', access_role=AccessRole.INSPECTOR)
    ...
    def test_issued_ppe_uses_replacement_date_logic(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            create_ppe_record(context.database_path, '0001', 'Окуляри', True, True, '2024-01-01', '2024-02-01', '1', 'Протерміновано', provision_status='issued', access_role=AccessRole.INSPECTOR)
    ...
    def test_not_required_ppe_does_not_create_expiry_problem(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            create_ppe_record(context.database_path, '0001', 'Фартух', False, False, '2024-01-01', '2024-02-01', '1', 'Не потрібен', provision_status='not_required', access_role=AccessRole.INSPECTOR)
            ...
            rows = connection.execute("SELECT COUNT(*) FROM notifications WHERE source_module = 'ppe.registry' AND message_text LIKE '%Фартух%'").fetchone()
            connection.close()
    ...
    def test_not_checked_compliance_creates_warning_but_not_critical(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            context = initialize_application(build_application_paths(Path(temporary_directory)))
            create_ppe_record(context.database_path, '0001', 'Рукавиці', True, True, '2026-05-01', '2026-12-01', '1', 'Без перевірки', provision_status='issued', compliance_check_state='not_checked', access_role=AccessRole.INSPECTOR)
            connection = sqlite3.connect(context.database_path)
