...
def build_save_mail_settings_handler(database_path: Path, daily_report_enabled_var: BooleanVar, smtp_host_var: StringVar, smtp_port_var: StringVar, smtp_username_var: StringVar, smtp_password_var: StringVar, sender_email_var: StringVar, recipient_email_var: StringVar, use_tls_var: BooleanVar, last_sent_date: str, on_success: Callable[[], None]) -> Callable[[], None]:
    ...
    def save_current_mail_settings() -> None:
        ...
        if smtp_port < 0:
            ...
            return
        save_mail_settings(database_path, MailSettings(daily_report_enabled=daily_report_enabled_var.get(), smtp_host=smtp_host_var.get().strip(), smtp_port=smtp_port, smtp_username=smtp_username_var.get().strip(), smtp_password=smtp_password_var.get(), sender_email=sender_email_var.get().strip(), recipient_email=recipient_email_var.get().strip(), use_tls=use_tls_var.get(), last_sent_date=last_sent_date), access_role=AccessRole.INSPECTOR)
