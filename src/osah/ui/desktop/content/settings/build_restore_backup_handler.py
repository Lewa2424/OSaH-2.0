...
def build_restore_backup_handler(database_path: Path, selected_backup_var: StringVar, on_success: Callable[[], None]) -> Callable[[], None]:
    ...
    def restore_selected_backup() -> None:
        ...
        if not selected_backup_file_name:
            ...
            return
        if not messagebox.askyesno('Підтвердження відновлення', 'Перед відновленням буде створено страховочну копію поточного стану. Продовжити?'):
            return
        backup_snapshots = load_backup_registry(database_path)
        backup_snapshot = next((candidate_snapshot for candidate_snapshot in backup_snapshots if candidate_snapshot.file_name == selected_backup_file_name), None)
        if backup_snapshot is None:
            messagebox.showerror('Помилка відновлення', 'Обрану резервну копію не знайдено.')
            return
        try:
            safety_backup_path = restore_backup_snapshot(database_path, backup_snapshot.file_path, access_role=AccessRole.INSPECTOR)
        except ValueError as error:
            messagebox.showerror('Помилка відновлення', str(error))
            return
        messagebox.showinfo('Відновлення завершено', f'Систему відновлено з копії {backup_snapshot.file_name}. Страховочна копія: {safety_backup_path.name}')
        on_success()
