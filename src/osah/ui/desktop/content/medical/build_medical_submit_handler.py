...
def build_medical_submit_handler(database_path: str, selected_employee_var: StringVar, valid_from_var: StringVar, valid_until_var: StringVar, medical_decision_var: StringVar, restriction_note_var: StringVar, on_success: Callable[[], None]) -> Callable[[], None]:
    ...
    def save_medical_record() -> None:
        ...
        """
        try:
            create_medical_record(database_path=database_path, employee_personnel_number=extract_medical_employee_number(selected_employee_var), valid_from_text=valid_from_var.get(), valid_until_text=valid_until_var.get(), medical_decision=extract_medical_decision_value(medical_decision_var.get()), restriction_note=restriction_note_var.get(), access_role=AccessRole.INSPECTOR)
