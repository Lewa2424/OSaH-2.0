...
def build_training_submit_handler(database_path: str, selected_employee_var: StringVar, training_type_var: StringVar, event_date_var: StringVar, next_control_date_var: StringVar, conducted_by_var: StringVar, note_var: StringVar, on_success: Callable[[], None]) -> Callable[[], None]:
    ...
    def save_training() -> None:
        ...
        """
        try:
            create_current_training_record(database_path=database_path, employee_personnel_number=extract_personnel_number(selected_employee_var), training_type=extract_training_type_value(training_type_var.get()), event_date_text=event_date_var.get(), next_control_date_text=next_control_date_var.get(), conducted_by=conducted_by_var.get(), note_text=note_var.get(), access_role=AccessRole.INSPECTOR)
