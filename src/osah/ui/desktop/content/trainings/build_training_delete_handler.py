...
def build_training_delete_handler(database_path: str, selected_record_var: StringVar, on_success: Callable[[], None]) -> Callable[[], None]:
    ...
    def delete_training() -> None:
        ...
        if not messagebox.askyesno('Підтвердження видалення', 'Видалити вибраний запис інструктажу?'):
            return
        try:
            delete_training_record(database_path=database_path, record_id=record_id, access_role=AccessRole.INSPECTOR)
